...
### Rmarkdown basic idea

- mingle code and text in document
- develop ideas over time with notes to self in rough document
- later format a refined document to share with others
...
work for future (self) reference.

- old way
    + write R script with code and comments
    + email collaborator with separate result and graph files
    + confusion over which graph was which
- middle way
    + formal reports (Word or LaTeX)
    + much time getting figures to look right (page breaks)
    + often revert back to old way to save time
- new way
    + Rmarkdown as HTML web page (one file)
    + allows tall figures and scrolling
    + can easily output as PDF or WORD if desired later
...
![R Markdown dialog box](img/New_R_Markdown.png)<br/>
...
See **Help → Markdown Quick Reference** for help.
...
Ideally, analysis reports are _reproducible_ documents: If an
...
function in my [R/broman](https://github.com/kbroman/broman) package handles
this.
